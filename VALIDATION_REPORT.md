...
> Sections 3-11 record the original equation-reconstruction audit and are
> retained for comparison. The current runners use the Hallow Table 3 joint
> calibration; its targets, fitted parameters, and current results are in
> `CALIBRATION_REPORT.md`.

## 1. Historical reconstruction status

The historical reconstruction passed equation-level, coupling, equilibrium,
stiff-integration, and cross-solver checks. Those results established
mathematical consistency and exposed the need for calibration. The separate
calibrated profile now matches the selected healthy baseline targets exactly;
this report does not overwrite the older audit values below.
...
All scripts completed successfully after calibration. The unit-test suite now
contains 20 passing tests: 11 standalone kidney tests and 9 cardiorenal tests.

## 3. Historical direct equation and interface checks
...
## 5. Historical pre-calibration coupled equilibrium verification
...
## 6. Historical mismatch that motivated the joint calibration
