...
For the current research model, use:
...
This selects the corrected Hallow kidney, baseline-normalized dynamic renin
feedback, reference-centered cardiovascular closure, and fixed CVP-Bowman
slope. RIHP remains off.

For source comparison without the novelty, use
...
## 10. Recommended CVP-enabled full equilibrium

Use this state for chronic experiments with
`cvp_extended_cardiorenal_parameters()` and baseline external inputs.
