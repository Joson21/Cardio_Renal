...
def run_validation(
    ...
    *,
    ...
    if (
        equilibrium_state is not None
        and model.p.kidney.renin_md_reference_mEq_per_min is not None
    ):
        raas_state = np.asarray(equilibrium_state, dtype=float)[:14]
    else:
        raas_state = kidney_y0
    ...
    raas_error = max(
        abs(raas_state[2 + index] - raas[name])
    ...
    if model.p.cardiovascular.closure_mode == "baseline_centered":
        checks[1:1] = [
            ValidationCheck(
                ...
                abs(
                    output["p_ma_mmHg"]
                    - model.p.cardiovascular.p_ma_ref_mmHg
                )
                < 1.0e-9,
                output["p_ma_mmHg"],
                1.0e-9,
                "The centered closure must preserve its configured MAP reference.",
            ),
            ValidationCheck(
                "centered_reference_pra",
                abs(
                    output["p_ra_mmHg"]
                    - model.p.cardiovascular.p_ra_ref_mmHg
                )
                < 1.0e-9,
                output["p_ra_mmHg"],
                1.0e-9,
                "The centered closure must preserve its configured Pra reference.",
            ),
            ValidationCheck(
                "centered_reference_cardiac_output",
                abs(
                    output["cardiac_output_L_per_min"]
                    - model.p.cardiovascular.cardiac_output_ref_L_per_min
                )
                < 1.0e-9,
                output["cardiac_output_L_per_min"],
                1.0e-9,
                "The centered closure must preserve its configured cardiac-output reference.",
            ),
            ValidationCheck(
