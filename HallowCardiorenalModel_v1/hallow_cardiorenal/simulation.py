"""Stiff integration and export for the coupled model."""
...
def _time_grid(days: float, samples_per_day: int) -> np.ndarray:
    if days <= 0.0:
...
def simulate_coupled(
    ...
    *,
    ...
    initial = np.asarray(
        y0 if y0 is not None else initial_state(model.p),
        dtype=float,
    )
    evaluation_times = _time_grid(days, samples_per_day)
    solution = solve_ivp(
        model.rhs,
